...
import { useShroom, useToadFactory } from 'hooks/useContract'
...
const minimumShroomBalanceToMint = new BigNumber(MINT_COST).multipliedBy(new BigNumber(10).pow(18))

const Mint: React.FC = () => {
  const [toadId, setToadId] = useState(null)
  const { actions, minimumShroomRequired, allowance } = useProfileCreation()

  const { account } = useWeb3React()
  const shroomContract = useShroom()
  const toadFactoryContract = useToadFactory()
  const TranslateString = useI18n()
  const hasMinimumShroomRequired = useHasShroomBalance(minimumShroomBalanceToMint)
  const {
    isApproving,
    isApproved,
    isConfirmed,
    ...
    onRequiresApproval: async () => {
      ...
      try {
        const response = await shroomContract.methods.allowance(account, toadFactoryContract.options.address).call()
      ...
      <Card mb="24px">
        <CardBody>
          ...
          <Text as="p" mb="24px" color="textSubtle">
            {TranslateString(999, `Cost: ${MINT_COST} SHROOM`, { num: MINT_COST })}
          </Text>
          {nfts.map((nft) => {
            ...
            return (
              <SelectionCard
                ...
                disabled={isApproving || isConfirming || isConfirmed || !hasMinimumShroomRequired}
              >
          ...
          {!hasMinimumShroomRequired && (
            <Text color="failure" mb="16px">
              {TranslateString(1098, `A minimum of ${MINT_COST} SHROOM is required`)}
            </Text>
          )}
          <ApproveConfirmButtons
            isApproveDisabled={toadId === null || isConfirmed || isConfirming || isApproved}
            isApproving={isApproving}
            isConfirmDisabled={!isApproved || isConfirmed || !hasMinimumShroomRequired}
